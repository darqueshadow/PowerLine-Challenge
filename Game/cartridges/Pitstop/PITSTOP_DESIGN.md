...
- **Mode compliance:** boots in **Demo Mode** (`CONFIG.isHolodeck=false`); **Ctrl+Shift+B** Developer Mode hook with timed password prompt (phrase stored as a digest in `core/config.js` as `devModePasswordHash`; run `plcDigest("...")` to regenerate) → unlocks a screen-jump dev panel + watermark (internal testing per Law §0.15). All other key handling ignores modifier combos so the hook is never swallowed. Holodeck deferred to Publish Mode.
