...
| Code | Meaning | Duration | In game, to the bold trigger *(Addendum scale)* | Notes |
|---|---|---|---|---|
...
7. **VF's trigger cue** — "pops up and says 'Clear Fuel'" needs a concrete visual/audio spec once art direction starts. **Deferred** *(Addendum)*: not blocking a build. VF's 10–30 min duration is **confirmed** *(Addendum)*.
...
### Raised by Claude Code during the build (2026-09-16) — ruled in Draft 9 (2026-09-17), except D3
...
- ~~**D5. What units does the counting-up timer show?**~~ **Resolved** *(Rulings, draft 9)*: **game seconds**, the time that actually passes: VS goes bold at 00:20 and hatches about 00:25, and EOS and MB go bold at 01:00. Switched from the real-time equivalent (§4). *(Original question follows.)* Built: **real-time equivalent**, so VS goes bold at 10:00 and MB at 30:00, with the clock ticking 30 real seconds per game-second. The alternative shows game seconds (VS bold at 0:20) (`timerDisplay`).
