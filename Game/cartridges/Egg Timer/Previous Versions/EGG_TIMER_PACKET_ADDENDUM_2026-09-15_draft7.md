...
> `Downloads/Egg_Timer_Packet_Addendum_2026-09-15.md` (22:33 on 2026-09-16). Everything below the rule is the
> Addendum as written. This is **Chat's seventh draft**, and it's the one merged into the live
> `EGG_TIMER_CONTEXT_PACKET.md`. Drafts 2–6 are beside this file (`..._draft2.md` to `..._draft6.md`); the first
> draft wasn't filed.
>
> **What this draft changes against the sixth:** the build gate reads **CLEARED** ("Andrew authorized the build on
> 2026-09-16"). The disk id `eggtimer` is confirmed, with the hub entry added when the build starts (C8). There's
> no suggestion list for v1 (item 17). The pool's name stays pending but is OK to build around. The out-of-scope
> pause note is corrected (only The Aquanaut changes). A new **§10** answers C2–C6: the overtime timer, VF's
> egg, placement timing, spawn rate, placement points, mess spread, grid adjacency, wrong real codes, and the
> three small readings.
>
> **⚠️ Checked against the packet on filing, 2026-09-16 — two answers collide with text already in it (packet
> §11 C12, C13), plus small gaps (C14):**
> C4's "no time limit to place a CAV … the nest just waits" contradicts the locked rule that an ignored
> trigger auto-opens (§6). C2's timer that "keeps counting up" contradicts the packet's countdown timer (§4, §7).
