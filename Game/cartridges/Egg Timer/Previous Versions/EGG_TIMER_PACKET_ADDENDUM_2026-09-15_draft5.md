...
> `Downloads/Egg_Timer_Packet_Addendum_2026-09-15.md` (17:58 on 2026-09-16). Everything below the rule is the
> Addendum as written. This is **Chat's fifth draft**, and it's the one merged into the live
> `EGG_TIMER_CONTEXT_PACKET.md`. Drafts 2–4 are beside this file (`..._draft2.md` to `..._draft4.md`), each with
> its own note; the first draft wasn't filed.
>
> **What this draft changes against the fourth — only §9, which gains six bullets:** Command Boxes are the
> switcher's only entries for now. Tab and Shift+Tab (or the arrow keys) move the highlight, Enter confirms, and
> Escape closes without switching (answers Code's C10). With 1 box, Tab does nothing. The game keeps running
> while the switcher is open. The 1–4 choice is made on the mode-selection screen. Tab's different job in The
> Aquanaut is ruled fine (answers C9). New open item: whether Egg Timer needs its own autocomplete for CAV entry.
>
> **Checked on filing, 2026-09-16:** The Aquanaut does request browser fullscreen, where browsers keep Esc, but
> its code already claims Esc back with the Keyboard Lock API, so Egg Timer's Escape rule is buildable in a
> plain tab. **⚠️ One new flag (packet §11 C11):** Egg Timer has no pause at all, while Asteroid Command and The
> Aquanaut both pause on Esc.
